...
ACPlayer::ACPlayer()
{
	...
	CHelpers::GetAsset<USkeletalMesh>(&meshAsset, "SkeletalMesh'/Game/Frank_Slash_Pack/Frank_Assassin/Skeletal_Meshes/SK_Mannequin.SK_Mannequin'");
	GetMesh()->SetSkeletalMesh(meshAsset);

	TSubclassOf<UAnimInstance> animInstanceClass;
	CHelpers::GetClass<UAnimInstance>(&animInstanceClass, "AnimBlueprint'/Game/Character/ABP_CPlayer.ABP_CPlayer_C'");
	GetMesh()->SetAnimInstanceClass(animInstanceClass);


	// -> SpringArmComp
	SpringArm->SetRelativeLocation(FVector(0, 0, 250));
	SpringArm->SetRelativeRotation(FRotator(0, 130, 0));
	...
	GetCharacterMovement()->MaxWalkSpeed = Status->GetFastRun();
